...
# ⭐ ADDED: CORS middleware so your HTML website can call this microservice
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],     # allow all origins (fine for class projects)
...
def classify_mood(mood: str) -> str:
    ...
    Returns:
        The mood category ('positive', 'neutral', 'negative') 
        or 'undefined' if the mood does not match any category.
    """
    mood = mood.lower().strip()   # ⭐ ADDED: normalize input
    
    for category, moods in mood_dictionary.items():
        if mood in moods:
            return category
    
    return "neutral"   # ⭐ FIXED: fallback so nothing breaks
